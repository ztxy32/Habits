import { useRoute } from "@react-navigation/native"
import dayjs from "dayjs"
import { useEffect, useState } from "react"
import { View, ScrollView, Text, Alert } from "react-native"
import { BackButton } from "../components/BackButton"
import { CheckBox } from "../components/CheckBox"
import { Loading } from "../components/Loading"
import { ProgressBar } from "../components/ProgressBar"
import { HabitListEmpty } from "../components/HabitListEmpty"
import { api } from "../lib/axios"
import { GenerateProgressPercentage } from "../utils/generate-progress-percentage"
import clsx from "clsx"

interface Params {
  date: string
}
interface DayInfoProps{
  completedHabits: string[]
  possibleHabits: {
    id: string
    title: string
  }[]
}

export function Habit(){
  const [loading, setLoading] = useState(true)
  const [dayInfo, setDayInfo] = useState<DayInfoProps | null>(null)
  const [completedHabits, setCompletedHabits] = useState<string[]>([])
  const route = useRoute()
  const { date } = route.params as Params
  const parsedDate = dayjs(date)
  const isDateInPast = parsedDate.endOf("day").isBefore(new Date())
  const dayOfWeek = parsedDate.format("dddd")
  const dayAndMonth = parsedDate.format("DD/MM")

  const habitProgress = dayInfo?.possibleHabits.length ? GenerateProgressPercentage(dayInfo.possibleHabits.length, completedHabits.length) : 0

  async function fetchHabits(){
    try{
      setLoading(true)
      const response = await api.get("/day", {params: { date }})
      setDayInfo(response.data)
      setCompletedHabits(response.data.completedHabits)
    }catch(error){
      console.log(error)
      Alert.alert("Something went wrong...","error fetching the data...")
    }finally{
      setLoading(false)
    }
  }
  async function handleToggleCompleted(habitId: string){
    try{
      await api.patch(`/habits/${habitId}/toggle`)
      if(completedHabits.includes(habitId)){
        setCompletedHabits(prevState => prevState.filter(item => habitId !== item))
      }else{
        setCompletedHabits(prevState => [...prevState, habitId])
      }
    }catch(error){
      console.log(error)
      Alert.alert("Ops", "Não foi possível atualizar o status do hábito...")
    }
  }

  useEffect(() => {
    fetchHabits()
  },[])
  if(loading){
    return(
      <Loading/>
    )
  }

  return(
    <View className="flex-1 bg-background px-8 pt-16">
      <ScrollView 
        showsVerticalScrollIndicator={false}
        contentContainerStyle={{paddingBottom: 60}}
      >
        <BackButton/>
        <Text className="mt-6 text-zinc-400 font-semibold text-base lowercase">
          {dayOfWeek}
        </Text>
        <Text className="text-white font-extrabold text-3xl">
          {dayAndMonth}
        </Text>

        <ProgressBar progress={habitProgress}/>
        <View className={clsx("mt-6", {
          ["opacity-50"]: isDateInPast
        })}>
          {
            dayInfo?.possibleHabits ?
            dayInfo?.possibleHabits.map(item => 
              <CheckBox 
                title={item.title}
                checked={completedHabits.includes(item.id)}
                key={item.id}
                onPress={() => handleToggleCompleted(item.id)}
                disabled={isDateInPast}
              />  
            ) :
            <HabitListEmpty/>
            
          } 
        </View>
        {
          isDateInPast && 
          <Text className="text-white mt-10">
            Não podes alterar um hábito em uma data qual já se passou...
          </Text>
        }
      </ScrollView>

    </View>
  )
}